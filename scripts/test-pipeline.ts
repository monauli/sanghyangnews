...
(async () => {
  ...
  sc.slice(0, 20).forEach((a, i) => {
    console.log(`  ${String(i + 1).padStart(2)}. [${String(a.score).padStart(3)}] ${a.judul.slice(0, 56)}`);
    console.log(`      📍${a.location} ${a.reasons.join(' ')}${a.dupeOf ? ' ⚠️mirip' : ''}`);
  });

  console.log('\n  ── [4] LOKASI HANYA DI NAMA SUMBER ──');
  const palsu = sc.filter((a) => !a.judul.toLowerCase().includes(a.location));
  ...
  for (const k of WAJIB_TINGGI) {
    const m = sc.filter((a) => a.judul.toLowerCase().includes(k));
    const best = m.sort((x, y) => y.score - x.score)[0];
    if (!best) { console.log(`  🔴 "${k}" — HILANG dari hasil`); continue; }
    const ok = tierOf(best.score) === 'tinggi';
    console.log(`  ${ok ? '✅' : '🔴'} [${String(best.score).padStart(3)}] ${best.judul.slice(0, 56)}  (${m.length} artikel serumpun)`);
  }
  ...
  for (const k of HARUS_HILANG) {
    ...
    m.slice(0, 3).forEach((a) => console.log(`      [${a.score}] ${a.judul.slice(0, 56)}`));
  }
